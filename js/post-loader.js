...
function renderPost() {
    ...
    const tagsHTML = Array.isArray(metadata.tags) && metadata.tags.length > 0
        ? `<div class="post-tags">${metadata.tags.map(tag => `<span class="tag">${escapeHtml(tag)}</span>`).join('')}</div>`
        : '';
    ...
    container.innerHTML = `
        <article class="post-content">
            <div class="post-header">
                <h1>${escapeHtml(metadata.title || currentPost.filename.replace('.md', ''))}</h1>
                <div class="post-meta">
                    <span>${escapeHtml(metadata.date || new Date().toISOString().split('T')[0])}</span>
                    ${categoryHTML}
                </div>
                ${tagsHTML}
            </div>
...
function loadGiscus() {
    ...
    script.setAttribute('data-repo-id', 'YOUR_REPO_ID'); // 나중에 설정 필요
    script.setAttribute('data-category', 'General');
    script.setAttribute('data-category-id', 'YOUR_CATEGORY_ID'); // 나중에 설정 필요
